...
class CartUpdateAPIMixin(object):
    def update_cart(self, *args, **kwargs):
        ...
        if cart:
            ...
            item_added = False
            if item_id:
                item_instance = get_object_or_404(Cycle, id=item_id)
...
class TokenMixin(object):
    ...
    def create_token(self, data_dict):
        # we have to have bytes not str as in python 3 bytes has A-z t-/ char

        if isinstance(data_dict, dict):
            import json
            token = base64.b64encode(json.dumps(data_dict).encode())
            self.token = token
            return token
        else:
